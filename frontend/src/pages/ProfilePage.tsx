...
import { Camera, LogOut, Moon, Sun, Check, Phone } from 'lucide-react';
...
export function ProfilePage() {
  ...
  const handleAvatar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try { const updated = await api.uploadAvatar(file); updateUser(updated); } catch {}
  };

  const handleLanguageChange = async (lang: string) => {
    try {
      await api.setLanguage(lang);
      updateUser({ preferred_language: lang } as any);
    } catch {}
  };
  ...
  return (
    <div className="pb-20">
      ...
      <div className="px-4 py-6 max-w-sm mx-auto space-y-6">
        ...
        {/* Name */}
        <div>
          <label className="text-sm font-medium text-[var(--nexly-text-secondary)] mb-1.5 block">Name</label>
        ...
        {/* Status */}
        <div>
          <label className="text-sm font-medium text-[var(--nexly-text-secondary)] mb-1.5 block">Status</label>
        ...
        {/* Settings */}
        <div className="space-y-1 pt-4 border-t border-[var(--nexly-border)]">
          <p className="text-xs font-medium text-[var(--nexly-text-secondary)] px-1 mb-2 uppercase tracking-wider">Settings</p>

          {/* Language */}
          <LanguagePicker selected={user.preferred_language || 'en'} onSelect={handleLanguageChange} compact />
          ...
          <button onClick={toggle}
            ...
            <span className="text-[var(--nexly-text)] text-sm">{dark ? 'Light Mode' : 'Dark Mode'}</span>
          </button>

          {/* Call history */}
          <a href="/calls" className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[var(--nexly-border)]/30 transition-colors">
            <Phone size={20} className="text-[var(--nexly-text-secondary)]" />
            <span className="text-[var(--nexly-text)] text-sm">Call History</span>
          </a>
